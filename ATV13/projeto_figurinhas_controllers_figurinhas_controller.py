...
def cadastrar_oferta():
    ...
    if request.method == "POST":
        ...
        oferta = OfertaTroca(colecionador_id=colecionador_id, observacao=observacao or None)
        db.session.add(oferta)
        db.session.flush()
        item_oferece = ItemOferta(oferta_id=oferta.id, figurinha_id=figurinha_oferece_id, tipo="oferece", quantidade=1)
        item_deseja = ItemOferta(oferta_id=oferta.id, figurinha_id=figurinha_deseja_id, tipo="deseja", quantidade=1)
